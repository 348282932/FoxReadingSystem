...
using FoxReadingSystem.Migrations.SeedData;
...
namespace FoxReadingSystem.Tests
{
    public abstract class FoxReadingSystemTestBase : AbpIntegratedTestBase<FoxReadingSystemTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected FoxReadingSystemTestBase()
        {
        ...
        protected void UsingDbContext(Action<FoxReadingSystemDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<FoxReadingSystemDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<FoxReadingSystemDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<FoxReadingSystemDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<FoxReadingSystemDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FoxReadingSystemDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<FoxReadingSystemDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<FoxReadingSystemDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<FoxReadingSystemDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<FoxReadingSystemDbContext, Task<T>> func)
        {
            T result;
